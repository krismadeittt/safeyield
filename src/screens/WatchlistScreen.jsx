          ...
          {watchlist.map(item => {
            ...
            return (
              <div key={item.ticker}
                ...
                style={{ padding: '0.8rem', borderBottom: '1px solid var(--border-row)', cursor: 'pointer' }}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                  <div>
                    <span style={{ fontWeight: 700, color: 'var(--accent)', fontSize: '0.95rem' }}>{item.ticker}</span>
                    {p?.price > 0 && (
                      <span style={{ color: 'var(--text-primary)', marginLeft: 8, fontSize: '0.85rem' }}>
                        ${p.price.toFixed(2)}
                      </span>
                    )}
                  </div>
                ...
                <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: 8 }}>{item.name || item.ticker}</div>
            ...
            <tbody>
              {watchlist.map(item => {
                ...
                return (
                  ...
                  >
                    <td>
                      <span style={{ fontWeight: 600, color: 'var(--accent)' }}>{item.ticker}</span>
                      <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{item.name}</div>
                    </td>
                    <td style={{ color: 'var(--text-primary)' }}>
                      {p?.price > 0 ? `$${p.price.toFixed(2)}` : '—'}
                    </td>
                    <td style={{ color: p?.change > 0 ? 'var(--green)' : p?.change < 0 ? 'var(--red-muted)' : 'var(--text-muted)' }}>
